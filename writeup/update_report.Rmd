...
Further, during a presentation of the replication, it was brought up that when selecting only for those who passed the manipulation checked, we may introduce sampling bias such that those who are kept in the data set may be abnormally sensitive to racial stereotypes. As such, analyses will be run with both the full data set (see section titled "Exploratory analyses") and with those who failed the manipulation check taken out, and results will be reported and discussed for both versions of analysis.
...
summary(lm(leadev ~ T_race*T_gender*T_behavior, d.tidy2)) #main 3 way interaction (leader evaluations)

summary(lm(leadev ~ T_race*T_behavior, d.women2)) #2 way for women (leader evaluations)

summary(lm(leadev ~ T_race*T_behavior, d.men2)) #2 way for men (leader evaluations)

summary(lm(PvsS ~ T_race*T_gender*T_behavior,d.tidy2)) #3 way interaction (attribution of behavior)
summary(lm(PvsS ~ T_race*T_behavior,d.women2)) #2 way for women (attribution of behavior)
summary(lm(PvsS ~ T_race*T_behavior,d.men2)) #2 way for men (attribution of behavior)
summary(lm(PvsS ~ T_race,d.tidy2)) #main effect of race
...
summary(lm(leadev ~ T_race*T_behavior*pol_s, d.men))
...
First, we collapsed the five leader status evaluation ratings into one composite dependent variable (`leadev`), and we were able to achieve adequate reliability, $\alpha$ = .86. As such, we used this composite as the dependent variable in all subsequent analyses. 

In our replication, we found that there was a main effect of behavior on leader status evaluations, **b = `r round(rs1$coefficients[3,1],3)`, t(`r rs1$df[2]`) = `r round(rs1$coefficients[3,3], 2)`, p = `r round(rs1$coefficients[3,4], 3)`**. That is, on average, those in the communal condition (M = `r round(mean(d.tidy$leadev[d.tidy$T_behavior=="communal"]), 2)`, SD = `r round(sd(d.tidy$leadev[d.tidy$T_behavior=="communal"]), 2)`) were evaluated significantly more favorably than those in the dominant condition (M = `r round(mean(d.tidy$leadev[d.tidy$T_behavior=="dominant"]), 2)`, SD = `r round(sd(d.tidy$leadev[d.tidy$T_behavior=="dominant"]), 2)`). However, there was no significant three-way interaction between target race, target gender, and target behavior, **b = `r round(rs1$coefficients[7,1],3)`, t(`r rs1$df[2]`) = `r round(rs1$coefficients[7,3], 2)`, p = `r round(rs1$coefficients[7,4], 3)`**. Further, when we analyzed the separate two-way interactions for male and female targets, we found that for female targets, there was a significant main effect of behavior on leader status evaluations,  **b = `r round(rs2$coefficients[2,1],3)`, t(`r rs2$df[2]`) = `r round(rs2$coefficients[2,3], 2)`, p = `r round(rs2$coefficients[2,4], 3)`**. Those in the communal condition (M = `r round(mean(d.women$leadev[d.women$T_behavior=="communal"]), 2)`, SD = `r round(sd(d.women$leadev[d.women$T_behavior=="communal"]), 2)`) were rated significantly more favorably than those in the dominant condition (M = `r round(mean(d.women$leadev[d.women$T_behavior=="dominant"]), 2)`, SD = `r round(sd(d.women$leadev[d.women$T_behavior=="dominant"]), 2)`). However, there was no significant interaction between target race and target behavior, **b = `r round(rs2$coefficients[3,1],3)`, t(`r rs2$df[2]`) = `r round(rs2$coefficients[3,3], 2)`, p = `r round(rs2$coefficients[3,4], 3)`**. For male targets, there was also a significant main effect of behavior on leader status evaluations, **b = `r round(rs3$coefficients[2,1],3)`, t(`r rs3$df[2]`) = `r round(rs3$coefficients[2,3], 2)`, p = `r round(rs3$coefficients[2,4], 3)`**. Those in the communal condition (M = `r round(mean(d.men$leadev[d.men$T_behavior=="communal"]), 2)`, SD = `r round(sd(d.men$leadev[d.men$T_behavior=="communal"]), 2)`) were rated significantly more favorably than those in the dominant condition (M = `r round(mean(d.men$leadev[d.men$T_behavior=="dominant"]), 2)`, SD = `r round(sd(d.men$leadev[d.men$T_behavior=="dominant"]), 2)`). However, again, there was no significant interaction between target race and target behavior, **b = `r round(rs3$coefficients[3,1],3)`, t(`r rs3$df[2]`) = `r round(rs3$coefficients[3,3], 2)`, p = `r round(rs3$coefficients[3,4], 3)`**. 

Overall, with regards to Livingston et al.'s main findings of the effect of target race, target gender, and target behavior on leader status evaluations, 
